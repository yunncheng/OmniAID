...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    ...
    if args.resume:
        ...
        if optimizer != None and 'optimizer' in checkpoint and 'epoch' in checkpoint:
...
def profile_model(model_without_ddp, input_size, device, args):

    print("\n" + "="*30 + " Model Profiling " + "="*30)
    ...
    try:
        print("  Creating a model copy for profiling to avoid state_dict modification...")
        model_copy = copy.deepcopy(model_without_ddp)
        model_copy.eval()
        ...
        gflops = flops / 1e9
        print(f"  GFLOPs (Input: {input_size}x{input_size}): {gflops:.2f} G")
